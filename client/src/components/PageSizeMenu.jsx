    ...
    <Listbox value={pageSize} onChange={handlePageSizeChange} disabled={disabled}>
      <div className={`relative w-full ${className}`}>
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <Listbox.Options className="absolute z-10 mb-1 bottom-full max-h-60 w-full overflow-auto rounded-xl bg-white text-base shadow-[0_4px_10px_rgba(0,0,0,0.03)] focus:outline-none">
            {options.map((option) => (
              <Listbox.Option
                key={option.id}
